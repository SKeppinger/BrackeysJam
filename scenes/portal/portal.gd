...
func _on_body_entered(body):
	if body.is_in_group("bullet"):
		...
		body.global_position += partner.arrow.target_position.rotated(partner.arrow.rotation) * partner.scale.x
		body.rotation = partner.arrow.rotation + deg_to_rad(180)
		var dir = (partner.arrow.target_position - partner.arrow.position).rotated(partner.arrow.rotation)
